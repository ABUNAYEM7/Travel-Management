...
const Hotels = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (query.trim()) {
      ...
      list = list.filter((h) => h.city.toLowerCase().includes(q) || h.name.toLowerCase().includes(q));
    }
    return list;
  }, [active, query]);
    ...
    <section className="py-16 sm:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 rounded-xl border border-gray-200 bg-white p-4 sm:p-5 shadow-sm">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
            <div className="sm:col-span-2">
              <label className="block text-xs font-semibold text-gray-600">Destination / Hotel</label>
              <input
                value={query}
          ...
          {FILTERS.map((f) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-semibold transition
                ${active === f ? "bg-gray-900 text-white" : "bg-white border border-gray-200 text-gray-700 hover:bg-gray-50"}`}
            >
              {f}
            </button>
          ))}
        ...
        <div className="mt-10 grid gap-8 sm:gap-10 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map((h) => (
            <article key={h.id} className="group overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm">
              {/* Image */}
              ...
              <div className="p-5">
                ...
                <div className="mt-5 flex items-end justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-extrabold text-gray-900">${h.price}<span className="text-sm font-semibold text-gray-500"> / night</span></p>
                  </div>
                  <Link
                    to={`/hotels/${h.id}`}
        ...
        <div className="mt-16 rounded-2xl bg-gray-900 p-8 sm:p-10 lg:p-12 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="text-white">
            <h4 className="text-2xl font-extrabold">Need a package with flights?</h4>
            <p className="mt-1 text-white/70">Tell us your dates and wishlist—we’ll send a custom plan in 24h.</p>
          </div>
